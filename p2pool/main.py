...
import mogwai.p2p as mogwai_p2p, mogwai.data as mogwai_data
from mogwai import stratum, worker_interface, helper
...
class keypool():
    ...
    def paytotal(self):
        ...
        for i in xrange(len(pubkeys.keys)):
            self.payouttotal += node.get_current_txouts().get(mogwai_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)*1e-8
...
def main(args, net, datadir_path, merged_urls, worker_endpoint):
    try:
        ...
        @defer.inlineCallbacks
        def connect_p2p():
            # connect to mogwaid over mogwai-p2p
            print '''Testing mogwaid P2P connection to '%s:%s'...''' % (args.mogwaid_address, args.mogwaid_p2p_port)
            factory = mogwai_p2p.ClientFactory(net.PARENT)
            			
            reactor.connectTCP(args.mogwaid_address, args.mogwaid_p2p_port, factory)
            def long():
                print '''    ...taking a while. Common reasons for this include all of mogwaid's connection slots being used...'''
        ...
        if args.testnet: # establish p2p connection first if testnet so mogwaid can work without connections
            factory = yield connect_p2p()
        
        # connect to mogwaid over JSON-RPC and do initial getmemorypool
        url = '%s://%s:%i/' % ('https' if args.mogwaid_rpc_ssl else 'http', args.mogwaid_address, args.mogwaid_rpc_port)
        print '''Testing mogwaid RPC connection to '%s' with username '%s'...''' % (url, args.mogwaid_rpc_username)
        mogwaid = jsonrpc.HTTPProxy(url, dict(Authorization='Basic ' + base64.b64encode(args.mogwaid_rpc_username + ':' + args.mogwaid_rpc_password)), timeout=30)
        yield helper.check(mogwaid, net)
        temp_work = yield helper.getwork(mogwaid, net)
        
        mogwaid_getinfo_var = variable.Variable(None)
        @defer.inlineCallbacks
        def poll_warnings():
            mogwaid_getinfo_var.set((yield deferral.retry('Error while calling getinfo:')(mogwaid.rpc_getinfo)()))
        yield poll_warnings()
        ...
        if args.pubkey_hash is None and args.address != 'dynamic':
            ...
            if address is not None:
                res = yield deferral.retry('Error validating cached address:', 5)(lambda: mogwaid.rpc_validateaddress(address))()
                if not res['isvalid'] or not res['ismine']:
                    print '    Cached address is either invalid or not controlled by local mogwaid!'
                    address = None
            
            if address is None:
                print '    Getting payout address from mogwaid...'
                address = yield deferral.retry('Error getting payout address from mogwaid:', 5)(lambda: mogwaid.rpc_getaccountaddress('p2pool'))()
            
            with open(address_path, 'wb') as f:
                f.write(address)
            
            my_pubkey_hash = mogwai_data.address_to_pubkey_hash(address, net.PARENT)
            print '    ...success! Payout address:', mogwai_data.pubkey_hash_to_address(my_pubkey_hash, net.PARENT)
            print
            ...
            for i in xrange(args.numaddresses):
                address = yield deferral.retry('Error getting a dynamic address from mogwaid:', 5)(lambda: mogwaid.rpc_getnewaddress('p2pool'))()
                new_pubkey = mogwai_data.address_to_pubkey_hash(address, net.PARENT)
            ...
            for i in xrange(len(pubkeys.keys)):
                print '    ...payout %d: %s' % (i, mogwai_data.pubkey_hash_to_address(pubkeys.keys[i], net.PARENT),)
        ...
        node = p2pool_node.Node(factory, mogwaid, shares.values(), known_verified, net)
        ...
        wb = work.WorkerBridge(node, my_pubkey_hash, args.donation_percentage, merged_urls, args.worker_fee, args, pubkeys, mogwaid)
        web_root = web.get_web_root(wb, datadir_path, mogwaid_getinfo_var, static_dir=args.web_static)
                ...
                def signedOn(self):
                    ...
                    def new_share(share):
                        ...
                        if share.pow_hash <= share.header['bits'].target and abs(share.timestamp - time.time()) < 10*60:
                            ...
                            message = '\x02%s BLOCK FOUND by %s! %s%064x' % (net.NAME.upper(), mogwai_data.script2_to_address(share.new_script, net.PARENT), net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
        ...
        def status_thread():
            ...
            while True:
                ...
                try:
                    ...
                    my_shares_per_s = sum(datum['work']/dt/mogwai_data.target_to_average_attempts(datum['share_target']) for datum in datums)
                    ...
                    if height > 2:
                        ...
                        paytot = 0.0
                        for i in xrange(len(pubkeys.keys)):
                            curtot = node.get_current_txouts().get(mogwai_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)
                            paytot += curtot*1e-8
                        ...
                        this_str += '\n Pool: %sH/s Stale rate: %.1f%% Expected time to block: %s' % (
                            ...
                            100*stale_prop,
                            math.format_dt(2**256 / node.mogwaid_work.value['bits'].target / real_att_s),
                        )
                        
                        for warning in p2pool_data.get_warnings(node.tracker, node.best_share_var.value, net, mogwaid_getinfo_var.value, node.mogwaid_work.value):
                            print >>sys.stderr, '#'*40
