...
extra_includes = []
import p2pool.mogwai
import p2pool.networks
extra_includes.extend('p2pool.networks.' + x for x in p2pool.networks.nets)
import p2pool.mogwai.networks
extra_includes.extend('p2pool.mogwai.networks.' + x for x in p2pool.mogwai.networks.nets)
...
try:
    ...
    if im64:
        bundle = bundle + 2
    sys.argv[1:] = ['py2exe']
    setup(name='mogwai-p2pool',
        version=version,
        description='Peer-to-peer Mogwai mining pool',
        author='Randall Peltzer',
        author_email='randall.peltzer@protonmail.com',
        url='https://github.com/mogwaicoin/mogwai-p2pool/',
        data_files=[
            ('', ['README.md']),
...
dir_name = 'p2pool_mogwai_win' + win + '_' + version
